...
def resolve_attn_implementation(override_config: Optional[object] = None) -> str:
    """Resolve attention implementation with a safe fallback when flash-attn is unavailable."""
    attn_implementation = None
    if override_config is not None:
        if isinstance(override_config, dict):
            attn_implementation = override_config.get("attn_implementation")
            if attn_implementation is None and isinstance(override_config.get("model_config"), dict):
                attn_implementation = override_config["model_config"].get("attn_implementation")
        else:
            attn_implementation = getattr(override_config, "attn_implementation", None)
            if attn_implementation is None:
                attn_implementation = getattr(override_config, "_attn_implementation", None)
