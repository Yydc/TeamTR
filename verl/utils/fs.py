...
def copy_to_shm(src: str):
    ...
    # Use a lock to avoid multiple workers copying the same model simultaneously.
    try:
        from filelock import FileLock

        lock_root = "/tmp/verl-shm-locks"
        os.makedirs(lock_root, exist_ok=True)
        lock_path = os.path.join(lock_root, f"{hashlib.md5(src_abs.encode('utf-8')).hexdigest()}.lock")
        ...
        # Fallback to best-effort copy without locking.
        if os.path.exists(dest) and verify_copy(src, dest):
            return dest
        if os.path.exists(dest):
            if os.path.isdir(dest):
