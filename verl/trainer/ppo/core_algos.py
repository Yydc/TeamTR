...
def _metric_scalar(value: torch.Tensor | float) -> float:
    if isinstance(value, torch.Tensor):
        return value.detach().float().cpu().item()
    return float(value)


def _policy_metric_dict(
    ...
    metrics = {
        "actor/pg_clipfrac": _metric_scalar(pg_clipfrac),
        "actor/ppo_kl": _metric_scalar(ppo_kl),
        "actor/pg_clipfrac_lower": _metric_scalar(pg_clipfrac_lower),
    }
    if extra:
...
def compute_policy_loss_vanilla(
    ...
    _ = rollout_is_weights
    metrics = _policy_metric_dict(pg_clipfrac, ppo_kl, pg_clipfrac_lower)
...
def compute_policy_loss_gpg(
    ...
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
    _ = old_log_prob, config, rollout_is_weights
    metrics = _policy_metric_dict(torch.tensor(0.0), torch.tensor(0.0), torch.tensor(0.0))
    return pg_loss, metrics
...
def compute_policy_loss_clip_cov(
    ...
    _ = rollout_is_weights
    metrics = _policy_metric_dict(pg_clipfrac, ppo_kl, torch.tensor(0.0))
    return pg_loss, metrics
...
def compute_policy_loss_kl_cov(
    ...
    _ = rollout_is_weights
    metrics = _policy_metric_dict(torch.tensor(0.0), ppo_kl_abs, torch.tensor(0.0))
    metrics["actor/ppo_kl_abs"] = _metric_scalar(ppo_kl_abs)
    return pg_loss, metrics
