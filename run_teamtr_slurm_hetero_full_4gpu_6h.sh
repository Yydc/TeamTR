#!/bin/bash
#SBATCH --job-name=teamtr_hetero_full_4gpu_6h
...
# Env + experiment settings (hetero full run defaults).
...
export TEAMTR_CONFIG_NAME="${TEAMTR_CONFIG_NAME:-teamtr_hetero_full}"
export TEAMTR_EXPERIMENT_NAME="${TEAMTR_EXPERIMENT_NAME:-teamtr_hetero_full_4gpu_6h}"
export TEAMTR_N_GPUS_PER_NODE="${TEAMTR_N_GPUS_PER_NODE:-4}"
...
# Large-scale run overrides.
export TEAMTR_TOTAL_TRAINING_STEPS="${TEAMTR_TOTAL_TRAINING_STEPS:-1000}"
export TEAMTR_SAVE_FREQ="${TEAMTR_SAVE_FREQ:-100}"
export TEAMTR_TEST_FREQ="${TEAMTR_TEST_FREQ:-100}"
export TEAMTR_TRAIN_BATCH_SIZE="${TEAMTR_TRAIN_BATCH_SIZE:-8}"
export TEAMTR_GEN_BATCH_SIZE="${TEAMTR_GEN_BATCH_SIZE:-8}"
export TEAMTR_ROLLOUT_N="${TEAMTR_ROLLOUT_N:-1}"
export TEAMTR_PPO_MINI_BATCH_SIZE="${TEAMTR_PPO_MINI_BATCH_SIZE:-8}"
...
export TEAMTR_DISABLE_TORCH_COMPILE="${TEAMTR_DISABLE_TORCH_COMPILE:-1}"

# Heterogeneous model paths (comma-separated, 3 paths).
export TEAMTR_MODEL_PATHS="${TEAMTR_MODEL_PATHS:-${TEAMTR_MODELS_ROOT:-./models}/Qwen__Qwen3-1.7B,${TEAMTR_MODELS_ROOT:-./models}/Qwen__Qwen3-8B,${TEAMTR_MODELS_ROOT:-./models}/Qwen__Qwen3-14B}"
MODEL_OVERRIDE_ARGS=""
...
TORCH_COMPILE_OVERRIDE_ARGS=""
if [ "${TEAMTR_DISABLE_TORCH_COMPILE}" -eq 1 ]; then
  TORCH_COMPILE_OVERRIDE_ARGS="actor_rollout_ref.actor.use_torch_compile=false \
actor_rollout_ref.actor.fsdp_config.use_torch_compile=false \
actor_rollout_ref.ref.use_torch_compile=false \
actor_rollout_ref.ref.fsdp_config.use_torch_compile=false \
critic.fsdp_config.use_torch_compile=false"
fi
...
srun -- "${RUNNER[@]}" bash -c "export PYTHONPATH=${TEAMTR_PROJECT_ROOT:-./TeamTR}; \
  ...
  command -v python; \
  echo \"Launching TeamTR hetero full 4GPU 6h run...\"; \
